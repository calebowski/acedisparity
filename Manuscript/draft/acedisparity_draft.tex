...
Model-related issues can also arise for discrete traits, which are estimated using M\textit{k} \citep{pagel1994detecting,pagel1999maximum,lewis2001likelihood} or SSE models \citep{ng2014traits, holland2020accuracy}. %@@@keating line 81
...
Here we evaluate the efficacy of different \ase approaches in recovering true disparity values across different evolutionary models and fossil sampling densities using a simulation framework.
...
Constant birth-death parameters meant that the simulated trees were highly symmetrical (\autoref{fig:main trees colless dist}; \citealp{khurana2024limits}).
Given that empirical phylogenies tend to be imbalanced \citep{mooers1997inferring, harcourt2001imbalance}, we also simulated 100 birth-death trees with high asymmetry and ran the same pipeline (\autoref{fig:assymetrical trees colless dist}).
...
We also simulated BM with a trend parameter of 0.3, by which the ancestral trait value increased incrementally proportional to branch length.
...
Discrete trait evolution was simulated using a discrete-time Markov process, whereby the probability of state transition along each branch was proportional to the product of the transition probability and branch length.
...
Pre-ordination \ase estimation involves the estimation of the raw trait values for each node. 
We performed pre-ordination estimation of continuous characters under a BM model, whereby ancestral state values lie within a multivariate normal distribution that is estimated given the observed tip data and the shared evolutionary history among taxa \citep{schluter1997likelihood,rohlf2001comparative}.
...
For discrete characters, we used a strict majority rule with the state with the highest scaled likelihood selected at each node for each character.
...
In a scenario that there is uncertainty in a binary character at a specific node, e.g., 0.55 for state 0 and 0.45 for state 1, this distribution approach results in a 55\% probability of sampling state 0 in each matrix for the respective node (and 45\% chance of sampling state 1).
...
For density, we calculated the sum of variances and mean pairwise distance \citep{foote1991morphological,foote1992paleozoic}. 
...
Given that the dimensionality of discrete trait spaces varied across fossil sampling densities within the same replicate, we also ran disparity calculations with dimensionality in the true and estimated trait spaces constrained to the minimum number of axes reached across all fossil sampling levels, i.e., for the 50 living-taxa trees this equalled 48 axes.
...
We also performed analyses in which the distribution was retained, but each individual point returned by distributions was weighted 1/100th of the single point estimate.
...
The log-transformed relative disparity error was treated as the response variable. 
...
We also included tree size as a further non-interactive fixed effect after finding it had no significant interaction with the other predictors.
...
When averaged across all scenarios, ancestral state estimations improved estimates of disparity compared to excluding ancestral states.
...
However, the pattern that \ase methods improved estimates of disparity was context-specific (\autoref{fig:continuous heatmap}).
...
A three-way interaction between estimation method, fossil sampling and evolutionary model was significant ($F_{32,66976} = 60,~ p < .001$) and the \textit{post-hoc} EMM can be used to identify the optimal method for models at each fossil sampling density (Table S2).
...
Notably, we found that fossil sampling was the strongest driver of recovery of an accurate disparity value for continuous traits ($F_{4,66976} = 9169, ~ p < .001$), as each increase in fossil sampling density resulted in a significant reduction in disparity error.
...
We found that the hierarchy of methods across continuous trait models and fossil sampling densities were largely congruent under the alternative 'tips+nodes' fossil sampling scheme, albeit we found that excluding ancestral states was marginally better performing when using the sum of quantiles metric on BM trait data (\autoref{fig:Continuous nodes tips boxplot}).
%% add asymmetry of trees here
...
Similarly to continuous traits, the three-way interaction between estimation method, fossil sampling and transition rate explained much of the variation in the data ($F_{32,66976} = 588, ~p < .001$; Table S4).
...
This pattern was transition rate-dependent, however, as under a slow rate the pre-ordination distribution method was the best performing method across all fossil sampling densities.
...
Again, while the disparity metric was a significant driver to overall accuracy, it had little effect on the hierarchy of methods defined by the interaction between transition rate and fossil sampling density, signified by a weak four-way interaction ($F_{64,66976} = 10,~ p < .001$).
...
\subsubsection{Sensitivity tests} 
Our findings did not differ under the alternative fossil sampling scheme under which nodes were sampled as well as tips.  %@@@add figure reference
...
For discrete characters, we found an inverse pattern in which distribution methods were optimal at low fossil sampling densities and point estimates were optimal at high fossil sampling densities.
...
In the case of slow transition rates, we face a paradoxical situation in which the root state can be easily estimated, yet few transitions make the rate of change hard to estimate \citep{gascuel2020darwinian}.
