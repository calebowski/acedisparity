...
\begin{abstract}
\noindent
Morphological traits and phylogenies are routinely paired to investigate macroevolutionary patterns during disparity analyses.
However, incomplete fossil sampling can distort disparity estimates, obscuring true evolutionary signals.
Ancestral state estimations can be used for both discrete and continuous traits to extend these analyses beyond incomplete fossil data, but the inevitable uncertainty in estimates complicates their integration into disparity analyses.
Determining the most robust workflow for treating this uncertainty remains a critical methodological challenge.
...
Ancestral state estimation generally improved recovery of true disparity relative to tip-only analyses, though the optimal method depended on the interaction between trait model and fossil sampling density. 
...
Ultimately, fossil sampling density was a stronger predictor of disparity accuracy than estimation method choice, underscoring that methodologies are only as powerful as the data provided.
...
Morphological traits and phylogenies are routinely used to investigate macroevolutionary patterns, yet incomplete fossil sampling can systematically distort disparity estimates, obscuring true evolutionary signal. 
Ancestral state estimation offers a means to recover missing data for both discrete and continuous traits, but the uncertainty inherent in these estimates complicates their integration into disparity analyses. 
Determining the most robust workflow for treating this uncertainty remains a critical methodological challenge.
Using simulations, we evaluated two axes of methodological choice — pre- versus post-ordination estimation, and point estimate versus probabilistic approaches — across varying trait models, fossil sampling densities, and disparity metrics. 
Ancestral state estimation consistently improved recovery of true disparity relative to tip-only analyses, though the optimal method depended on the interaction between trait model and fossil sampling density. 
For continuous traits, probabilistic approaches were most accurate overall, but proved sensitive to model misspecification when fossil sampling was low. 
For discrete traits, pre-ordination methods were most reliable; probabilistic approaches outperformed point estimates under low sampling, while point estimates became increasingly competitive as sampling density increased.
Strikingly, fossil sampling density was a stronger predictor of disparity accuracy than estimation method choice, underscoring that methodological sophistication cannot compensate for poor sampling.
Based on these results, we provide a practical decision framework to help researchers select the most appropriate workflow given the sampling and trait characteristics of their data.
...
\ASE is the method of estimating trait value(s) in a set of common ancestors, i.e., nodes on a phylogenetic tree, given a set of observed tip data \citep{schluter1997likelihood,cunningham1998reconstructing,revell2025ancestral}.
...
These estimated states can be combined with the observed data to generate a more complete trait space, helping to fill the gaps left by an incomplete fossil record and, in theory, improve our ability to recover the true disparity of a group \citep{sidlauskas2008continuous,brusatte2011phylogenetic,lloyd2018journeys}.
...
Model-related issues can also arise for discrete traits, which are estimated using the M\textit{k} model \citep{lewis2001likelihood,pagel1994detecting,pagel1999maximum}.
...
These were then compared to a null model bypassing ancestral state estimation (Fig. \ref{fig:pipeline}, Step 4).
