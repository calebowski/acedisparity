#!/bin/bash
#SBATCH --job-name=pack_folders_continuous
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
cd /mnt/parscratch/users/bip24cns/acedisparity/continuous/
...
for folder in */; do
    ...
    echo "Starting to pack: $clean_name -> continuous_${clean_name}.tar"
    
    # 4. Run the tar command (using -cf for Create File, dropping the verbose output)
    tar -cf "continuous_${clean_name}.tar" "$clean_name"
...
# rsync -avP --inplace --no-owner --no-group --no-perms /mnt/parscratch/users/bip24cns/acedisparity/continuous/*.tar /shared/guillerme_group/User/bip24cns/acedisparity/continuous/
