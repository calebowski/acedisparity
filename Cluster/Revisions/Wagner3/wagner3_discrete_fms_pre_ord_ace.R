...
base_path <- "/mnt/parscratch/users/bip24cns/acedisparity/revisions/discrete_wagner3/"
...
fossilised_high <- lapply(matrices, fossil.pres.duration, trees = crown_tree, sample.prob = 0.5, type = "discrete", seed = set_seed)
all_fossil <- lapply(matrices, fossil.pres.alt, trees = crown_tree, preservation = 1.0, type = "discrete", seed = set_seed) ## use this function because applying rates is leads to -Inf lambda value.
fossilised_med <- lapply(matrices, fossil.pres.duration, trees = crown_tree, sample.prob = 0.15, type = "discrete", seed = set_seed)
fossilised_low <- lapply(matrices, fossil.pres.duration, trees = crown_tree, sample.prob = 0.05, type = "discrete", seed = set_seed)
