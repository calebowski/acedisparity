...
# fossil_matrices <- lapply(fossil_matrices, lapply, function(x) x[!rownames(x) %in% extant, , drop = FALSE])
fossil_matrices <- lapply(fossil_matrices, function(rate_data) {
  rate_data$living <- NULL

  lapply(rate_data, function(level_data) {
    level_data$matrix <- level_data$matrix[
      !rownames(level_data$matrix) %in% extant,
      ,
      drop = FALSE
    ]
    level_data
...
# point_anc <- lapply(point_anc, lapply, function(x) x[!rownames(x) %in% extant, , drop = FALSE])
point_anc <- lapply(point_anc, function(rate_data) {
  rate_data$living <- NULL

  lapply(rate_data, function(level_matrix) {
    level_matrix[
      !rownames(level_matrix) %in% extant,
      ,
      drop = FALSE
    ]
  })
...
ace_errors <- Map(function(rate_true, rate_ace){
        lapply(rate_ace, function(fossil_level){
            (fossil_level - rate_true) / rate_true
        })
