...
for (size in tree_sizes) {
  ...
  for (method in methods) {
    ...
    for(i in 1:100) {
      file_path <- file.path("..", "Data", "discrete", size, 
                            "disparity", "raw",
...
results_df_long$ratio <- results_df_long$error + 1


# lmm_model <- lmer(abs_error ~ model * method * fossil_sampling * metric + tree_size +
#                                   (1|tree_unique_id),
#                     data = results_clean, REML = TRUE)
...
## glmm
# Create the is_failure column
results_df_long$is_failure <- results_df_long$abs_error > 1

# Calculate failure rate (mean) and number of failures (sum) 
# using aggregate for the grouping variables
agg_mean <- aggregate(is_failure ~ model + method + metric + tree_size, 
                      data = results_df_long, FUN = mean)
agg_sum  <- aggregate(is_failure ~ model + method + metric + tree_size, 
                      data = results_df_long, FUN = sum)

# Rename the aggregated columns
colnames(agg_mean)[ncol(agg_mean)] <- "failure_rate"
colnames(agg_sum)[ncol(agg_sum)] <- "n_failures"

# Merge the two summaries together
failure_summary <- merge(agg_mean, agg_sum)

# Filter for rows where n_failures > 0
failure_summary[failure_summary$n_failures > 0, ]

# Fit Beta GLMM on clean data only
results_clean <- results_df_long[!results_df_long$is_failure, ]




# n <- nrow(results_clean)
# results_clean$abs_error_sq <- (results_clean$abs_error * (n - 1) + 0.5) / n

library(glmmTMB)
glmm_model <- glmmTMB(abs_error ~ model * method * fossil_sampling * metric + tree_size +
                        (1 | tree_unique_id),
                      family = beta_family(link = "logit"),
                      dispformula = ~metric,  # allow variance to differ by metric
                      data = results_clean,
...
gamma_model <- glmmTMB(
 abs_error ~ model * method * fossil_sampling * metric + tree_size +
                        (1 | tree_unique_id),
...
# Simulate the residuals based on your fitted model
# (This may take a minute or two since you have a large dataset)
simulated_res <- simulateResiduals(fittedModel = glmm_model)
simulated_res_gamma <- simulateResiduals(fittedModel = gamma_model)
simulated_res_lognormal <- simulateResiduals(fittedModel = lognormal_model)


# Generate the diagnostic plots
plot(simulated_res_gamma)
plot(simulated_res_lognormal)


resid <- residuals(glmm_model, type = "pearson")
fitted <- fitted(glmm_model)
plot(fitted, resid,
...
results_pairwise <- results_df_long[results_df_long$metric == "pairwise", ]
results_sum_var <- results_df_long[results_df_long$metric == "sum_var", ]
results_sum_quant <- results_df_long[results_df_long$metric == "sum_quant", ]

lmm_pairwise <-  lmer(log_abs_error ~ model * method * fossil_sampling  + tree_size +
                                  (1|tree_unique_id),
                    data = results_pairwise, REML = TRUE)
lmm_sum_var <-  lmer(log_abs_error ~ model * method * fossil_sampling+ tree_size +
                                  (1|tree_unique_id),
                    data = results_sum_var, REML = TRUE)

lmm_sum_quant  <-  lmer(log_abs_error ~ model * method * fossil_sampling + tree_size +
                                  (1|tree_unique_id),
                    data = results_sum_quant, REML = TRUE)




# results_pairwise <- results_clean %>% filter(metric == "pairwise")
# results_sum_quant <- results_clean %>% filter(metric == "sum_quant")
# results_sum_var <- results_clean %>% filter(metric == "sum_var")

# fit_beta <- function(df) {
#   glmmTMB(abs_error_sq ~ model * method * fossil_sampling + tree_size +
#             (1 | tree_unique_id),
#           family = beta_family(link = "logit"),
#           data = df,
#           control = glmmTMBControl(optCtrl = list(iter.max = 10000,
...
# m_pairwise  <- fit_beta(results_pairwise)
# m_sum_quant <- fit_beta(results_sum_quant)
# m_sum_var   <- fit_beta(results_sum_var)

# # Check each
# plot(simulateResiduals(m_pairwise))
# plot(simulateResiduals(m_sum_quant))
# plot(simulateResiduals(m_sum_var))


art_model <- art(abs_error ~ model * method * fossil_sampling * metric * tree_size + 
                 (1|tree_unique_id), 
                 data = results_df_long)
anova(art_model)
...
# QQ plot
qqnorm(resid(lmm_sum_quant), main = "Q-Q Plot of Residuals")
qqline(resid(lmm_sum_quant), col = "red")

# Residuals vs Fitted
plot(fitted(lmm_sum_quant), resid(lmm_sum_quant),
     xlab = "Fitted Values", ylab = "Residuals",
     main = "Residuals vs Fitted")
abline(h = 0, col = "red", lty = 2)

# Scale-Location
plot(fitted(lmm_sum_quant), sqrt(abs(resid(lmm_sum_quant))),
     xlab = "Fitted Values", ylab = "Square Root of |Residuals|",
     main = "Scale-Location")

# Histogram of residuals
hist(resid(lmm_sum_quant), breaks = 50, main = "Histogram of residuals",
     xlab = "Residuals")
dev.off()
