#!/bin/bash
#SBATCH --job-name=pack_folders_discrete
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
cd /mnt/parscratch/users/bip24cns/acedisparity/discrete/crown/50t/
...
for folder in */; do
    ...
    echo "Starting to pack: $clean_name -> discrete_50t_${clean_name}.tar"
    
    # 4. Run the tar command (using -cf for Create File, dropping the verbose output)
    tar -cf "discrete_50t_${clean_name}.tar" "$clean_name"
