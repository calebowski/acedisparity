#!/bin/bash
#SBATCH --job-name=pack_folders_discrete_100t
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
cd /mnt/parscratch/users/bip24cns/acedisparity/discrete/crown/100t/
...
for folder in */; do
    ...
    echo "Starting to pack: $clean_name -> discrete_100t_${clean_name}.tar"
    
    # 4. Run the tar command (using -cf for Create File, dropping the verbose output)
    tar -cf "discrete_100t_${clean_name}.tar" "$clean_name"
...
# rsync -avP --inplace --no-owner --no-group --no-perms /mnt/parscratch/users/bip24cns/acedisparity/discrete/crown/100t/*.tar /shared/guillerme_group/User/bip24cns/acedisparity/discrete/100t
