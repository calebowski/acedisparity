...
results_df_long$model <- factor(
  ...
  labels = c("BM", "BM + trend", "OU (weak)", "OU", "OU (shifting)")
)
...
results_df_long  <- results_df_long %>% filter(model %in% c("BM", "BM + trend", "OU"))
...
# # Print model summary
lmm_model <- readRDS("../Data/cluster/continuous/lmm/lmm_continuous_3_model_final.rds")
lmm_model <- readRDS("../Data/cluster/continuous/lmm/lmm_continuous_3_model_final.rds")
terms <- readRDS("../Data/cluster/continuous/lmm/final_model_terms.rds")
