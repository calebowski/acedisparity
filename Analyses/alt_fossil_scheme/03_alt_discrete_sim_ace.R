...
dir.create("../Data/discrete_wagner_1", recursive = TRUE, showWarnings = FALSE)
dir.create("../Data/discrete_wagner_1/matrices", recursive = TRUE, showWarnings = FALSE)
dir.create("../Data/discrete_wagner_1/ord", recursive = TRUE, showWarnings = FALSE)
dir.create("../Data/discrete_wagner_1/ace", recursive = TRUE, showWarnings = FALSE)
...
run.sim.discrete.ace <- function(tree_size, replicate_id, samples = 100) {
    ...
    saveRDS(matrices, sprintf("../Data/discrete_wagner_1/matrices/matrices_%st_%03d.rds", tree_size, replicate_id))
    ...
    saveRDS(fossil_matrices, sprintf("../Data/discrete_wagner_1/matrices/fossil_matrices_%st_%03d.rds", tree_size,replicate_id))

    fossil_trees <- lapply(fossil_matrices, lapply, function(level) {
    tree <- level$tree
    })

    saveRDS(fossil_trees, sprintf("../Data/discrete_wagner_1/fossil_trees_%st_%03d.rds", tree_size, replicate_id))
    ...
    sample_pre_ord_ace <- lapply(res_pre_ord_ace, lapply, multi.ace, output = "combined.matrix", ml.collapse = list(type = "sample", sample = 100))

    
    saveRDS(res_pre_ord_ace, sprintf("../Data/discrete_wagner_1/ace/pre_ord_ace_%st_%03d.rds", tree_size, replicate_id))
    saveRDS(point_pre_ord_ace, sprintf("../Data/discrete_wagner_1/ace/pre_ord_point_%st_%03d.rds", tree_size, replicate_id))
    saveRDS(sample_pre_ord_ace, sprintf("../Data/discrete_wagner_1/ace/pre_ord_sample_%st_%03d.rds", tree_size, replicate_id))
    ...
    saveRDS(ord_pre_sample, sprintf("../Data/discrete_wagner_1/ord/ord_pre_sample_%st_%03d.rds", tree_size, replicate_id))
    saveRDS(ord_pre_point, sprintf("../Data/discrete_wagner_1/ord/ord_pre_point_%st_%03d.rds",tree_size, replicate_id))
    saveRDS(ord_no_ace, sprintf("../Data/discrete_wagner_1/ord/ord_no_ace_%st_%03d.rds", tree_size, replicate_id))
    saveRDS(ord_true, sprintf("../Data/discrete_wagner_1/ord/ord_true_%st_%03d.rds", tree_size, replicate_id))
    saveRDS(post_ord_ace, sprintf("../Data/discrete_wagner_1/ace/post_ord_ace_%st_%03d.rds", tree_size, replicate_id))
    saveRDS(point_post_ord_ace, sprintf("../Data/discrete_wagner_1/ord/ord_post_point_%st_%03d.rds", tree_size, replicate_id))
    saveRDS(sample_post_ord_ace, sprintf("../Data/discrete_wagner_1/ord/ord_post_sample_%st_%03d.rds", tree_size, replicate_id))
}


# Single replicate for testing reproducibility --------------------------------------
run.sim.discrete.ace(tree_size = 50, replicate_id = 1, samples = 10) ## lower sampling for computational speed
