...
dir.create("../Data/trees_alternative", recursive = TRUE, showWarnings = FALSE)
dir.create("../Data/trees_alternative/metadata", recursive = TRUE, showWarnings = FALSE)
...
for(i in 1:n_replicates) {
  ...
  metadata_list <- lapply(sizes, function(living_tips) {
    ...
    repeat {
      # set.seed(seed_val)
      tree <- treats(stop.rule = list(max.living = living_tips), bd.params = bd_params, null.error = 1e6, save.steps = 1)
    ...
    tree_file <- sprintf("../Data/trees_alternative/tree_%st_%03d.tre", tree_size, replicate_id)
  ...
  metadata_file <- sprintf(file.path("../Data/trees_alternative", "metadata", "metadata_%03d.csv"), replicate_id)
...
tree_50 <- read.tree("../Data/trees_alternative/tree_50t_001.tre")
plot(ladderize(tree_50))
