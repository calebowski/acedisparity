...
for (size in tree_sizes) {
  ...
  for (method in methods) {
    ...
    for(i in 1:100) {
      file_path <- file.path("..", "Data", "continuous",
                            "disparity",
                            sprintf("%s_disparity_%s_%03d.rds", method, size, i)) ## access disparity errors calcualted from analysis `04_continuous_disparity.R`
...
colours <- c("BM" = "#F60018", "BM + trend" = "#E69F00", "OU" = "#009999")
