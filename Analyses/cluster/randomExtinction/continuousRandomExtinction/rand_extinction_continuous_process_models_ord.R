...
cat("=== STARTING ord_sample ===\n")
  ord_sample <- lapply(sample_anc, lapply, function(mat){
    prcomp(mat, scale = FALSE, center = TRUE)$x
  })


cat("=== STARTING ord_point ===\n")
  ord_point <- lapply(point_anc, function(mat){
    prcomp(mat, scale = FALSE, center = TRUE)$x
  })


cat("=== STARTING ord_no_ace ===\n")
  ord_no_ace <- lapply(fossil_matrices, function(mat){
    prcomp(mat, scale = FALSE, center = TRUE)$x
  })


cat("=== STARTING ord_true ===\n")
  ord_true <- prcomp(mat, scale = FALSE, center = TRUE)$x
...
cat("Ordinations saved for model", model_name,  "for replicate", replicate_id, "\n")
