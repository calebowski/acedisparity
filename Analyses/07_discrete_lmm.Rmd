...
dir.create("../Data/discrete/lmm", recursive = TRUE, showWarnings = FALSE)
dir.create("../Data/discrete/lmm/tables", recursive = TRUE, showWarnings = FALSE)

```



# Aggregated Discrete Linear mixed model
...
for (size in tree_sizes) {
  ...
  for (method in methods) {
    ...
    for(i in 1:n_replicates) {
      file_path <- file.path("..", "Data", "discrete", "disparity", sprintf("%s_disparity_%st_%03d.rds", method, size, i))
...
saveRDS(lmm_model, "../Data/discrete/lmm/lmm_model_four_way.rds")
lmm_model <- readRDS("../Data/discrete/lmm/lmm_model_four_way.rds")
...
```{R}
pdf("../Manuscript/figures/aggregated_discrete_diagnostic_plot.pdf")
par(mfrow = c(2, 2))
...
# Weighted Continuous linear mixed model
This lmm instead includes the entire distribution of the pre/post-ordination dist method and weights each point 1/100th of the correspondoing point estimate.

## Reassamble df and add weights
This block of code:
* reassambles `results_df_long`
* assigns weights to distribution errors
* factorizes variables
* adds `tree_unique_id` as before
...
dir.create("../Data/discrete/lmm/weighted")
dir.create("../Data/discrete/lmm/weighted/tables")
lmm_weighted_path <- "../Data/discrete/lmm/weighted/"
...
```{R}
pdf("../Manuscript/figures/weighted_discrete_diagnostic_plot.pdf")
par(mfrow = c(2, 2))
...
```{R}
sink(paste0(lmm_weighted_path, "model_summaries_four_way.txt"))
cat("\n Model Summary \n")
...
cat("\n ANOVA table type 3 due to unequal sample sizes \n")
print(xtable((anova(lmm_model_weighted, type = 3))))
...
cld_model <- cld(emm_model, Letters = letters, alpha = 0.05)
write.csv(cld_model, paste0(lmm_weighted_path, "tables/model_multcomp.csv"))
print(xtable(cld_model), 
      include.rownames = FALSE)
...
cld_method <- cld(emm_method, Letters = letters, alpha = 0.05)
write.csv(cld_method, paste0(lmm_weighted_path, "tables/method_multcomp.csv"))
print(xtable(cld_method), 
      include.rownames = FALSE)
...
cld_fossils <- cld(emm_fossils, Letters = letters, alpha = 0.05)
write.csv(cld_fossils, paste0(lmm_weighted_path, "tables/fossil_multcomp.csv"))
print(xtable(cld_fossils), include.rownames = FALSE)

cat("\n--- Metric Level Rankings ---\n")
emm_metric <- emmeans(lmm_model_weighted, ~ metric)
cld_metric <- cld(emm_metric, Letters = letters, alpha = 0.05)
write.csv(cld_metric, paste0(lmm_weighted_path, "tables/metric_multcomp.csv"))
print(xtable(cld_metric), include.rownames = FALSE)
...
write.csv(cld(emm_model_method, Letters = letters, alpha = 0.05), paste0(lmm_weighted_path, "tables/model_method_multcomp.csv"))
print(xtable(cld(emm_model_method, Letters = letters, alpha = 0.05)))

# Method × Fossil sampling interaction
cat("\n--- Method × Fossil sampling Interaction ---\n")
emm_method_fossil <- emmeans(lmm_model_weighted, ~ method * fossil_sampling)
write.csv(cld(emm_method_fossil, Letters = letters, alpha = 0.05), paste0(lmm_weighted_path, "tables/fossil_method_multcomp.csv"))
print(xtable(cld(emm_method_fossil, Letters = letters, alpha = 0.05), include.rownames = FALSE))
...
write.csv(cld(emm_method_metric, Letters = letters, alpha = 0.05), paste0(lmm_weighted_path, "tables/method_metric_multcomp.csv"))
print(xtable(cld(emm_method_metric, Letters = letters, alpha = 0.05), include.rownames = FALSE))
...
write.csv(cld(emm_three_way, Letters = letters, alpha = 0.05), paste0(lmm_weighted_path, "tables/method_model_fossil_multcomp.csv"))
print(xtable(cld(emm_three_way, Letters = letters, alpha = 0.05), include.rownames = FALSE))
...
# Print only a subset to avoid overwhelming output
write.csv(cld(emm_four_way, Letters = letters, alpha = 0.05), paste0(lmm_weighted_path, "tables/method_model_fossil_metric_multcomp.csv"))

by_method <- emmeans(lmm_model_weighted, ~ method | model * fossil_sampling * metric)
write.csv(cld(by_method, Letters = letters, alpha = 0.05), paste0(lmm_weighted_path, "tables/method_model_fossil_by_metric_multcomp.csv"))
by_method_cld <- cld(by_method, Letters = letters, alpha = 0.05)
print(xtable(by_method_cld, include.rownames = FALSE))
...
write.csv(as.data.frame(summary(ase_vs_noase)), paste0(lmm_weighted_path, "tables/contrast_ase_vs_noase.csv"))
write.csv(as.data.frame(summary(preord_vs_postord)), paste0(lmm_weighted_path, "tables/contrast_preord_vs_postord.csv"))
write.csv(as.data.frame(summary(point_vs_distribution)), paste0(lmm_weighted_path, "tables/contrast_point_vs_dist.csv"))

```

## Plot weighted emmean heatmap
...
# 1. Prepare the data
plot_df <- as.data.frame(emm_three_way)
emm_three_way <- emmeans(lmm_weighted_path, ~ method * model * fossil_sampling, type = "response")
plot_df <- as.data.frame(emm_three_way)
